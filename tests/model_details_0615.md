...
本设计方案详尽解析了 Scholar Agent V2.0 的系统架构、六大核心阶段的技术细节、核心参数指标、以及为攻克学术词汇错配、大模型幻觉和精度/召回率权衡问题而引入的闭环控制逻辑。
...
* **解决 Precision 稀释（F1 暴跌）的设计哲学**：
  ...
  * **语义聚类与脉络时间线**：
    利用大模型 Pro 级语义理解能力，针对这前 3 篇论文在技术方法（method_clusters）、发表时间节点（timeline）进行精细化聚类和里程碑梳理，并在 LLM 调用失败时由 `_create_fallback_synthesis()` 进行启发式本地兜底。
